...
void MissionPlanner::renderStoryboard() {
    ...
    if (totalDistance <= 0.0f) totalDistance = 1.0f;
    ...
    for (size_t i = 0; i < visible; ++i) {
        ...
        std::string metrics = "ALT " + std::to_string(static_cast<int>(std::round(workingMission_.waypoints[i].position.y))) + " m"
                            + "   |   DIST " + formatFloat(legKm, 1) + " km"
                            + "   |   HDG " + std::to_string(static_cast<int>(std::round(headingDeg))) + "°";
...
void MissionPlanner::loadMissionFromFile(const std::string& path) {
    ...
    auto extractString = [&](const std::string& key) {
        ...
        if (firstQuote == std::string::npos || secondQuote == std::string::npos) return std::string();
    ...
    auto extractFloat = [&](const std::string& key, float def) {
        ...
        while (valStart < content.size() && (content[valStart] == ' ' || content[valStart] == '\n' || content[valStart] == '\t')) ++valStart;
        size_t valEnd = valStart;
        while (valEnd < content.size() && content[valEnd] != ',' && content[valEnd] != '}' && content[valEnd] != ']') ++valEnd;
