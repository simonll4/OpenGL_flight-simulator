...
namespace dlfdm
{
                                                                             ...
                                                                             const ControlInputs &controls)
    {
        ...
        // p dot
        state_derv_.body_omega_dot.x = (c1 * r + c2 * p) * q + c3 * aero.body_moments.x + c4 * aero.body_moments.z;

        // q dot
        state_derv_.body_omega_dot.y = c5 * p * r - c6 * (p * p - r * r) + aero.body_moments.y / Iyy;

        // r dot
        state_derv_.body_omega_dot.z = (c8 * p - c2 * r) * q + c4 * aero.body_moments.x + c9 * aero.body_moments.z;
